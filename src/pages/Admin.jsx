  ...
  return (
    <div className="min-h-screen bg-ink flex items-center justify-center p-4">
      <div className="bg-white rounded-xl p-8 max-w-sm w-full text-center">
        ...
        <p className="text-stone-500 text-sm mb-6">Senoia PorchFest 2026</p>
        {(error ?? linkError) && (
...
function Dashboard({ user }) {
  ...
  function exportCsv() {
    // Same column layout as the car show export so the organizers can reuse
    // their spreadsheet muscle memory.
    ...
    // An active signup whose shift was deleted is still a real volunteer —
    // export it under a placeholder rather than let the printed roster
    // disagree with the on-screen totals.
    const knownShiftIds = new Set(sorted.map((s) => s.id))
    for (const v of signups ?? []) {
      if (!knownShiftIds.has(v.shiftId)) {
        rows.push(['(deleted shift)', '', '', v.firstName, v.lastName, v.email, v.phone])
      }
    }
    const csv = rows
      .map((r) => r.map((c) => {
    ...
    a.href = url
    a.download = `senoia-porchfest-signups-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            ...
            <select value={form.day} onChange={set('day')}
              ...
              <option value="2026-09-05">Sat 9/5 (Setup)</option>
              <option value="2026-09-06">Sun 9/6 (PorchFest)</option>
              <option value="2026-09-07">Mon 9/7 (Cleanup)</option>
            </select>
          </label>
          <label className="block">
